    ...
    def _draw_selected_dot_candidates(self) -> None:
        ...
        for candidate, color_name in [
            ...
            *((candidate, "purple") for candidate in nucleus.overlap_dot_candidates),
            ...
            radius = max(3.0, min(11.0 if is_overlap else 10.0 if is_large_red else 8.0, (float(candidate.area) ** 0.5) / 2.0 + 2.0))
            dot = QGraphicsEllipseItem(candidate.x - radius, candidate.y - radius, radius * 2.0, radius * 2.0)
            pen = QPen(QColor("orange" if is_large_red else color_name), 3 if (is_large_red or is_overlap) else 2)
            if is_large_red or is_overlap:
                pen.setStyle(Qt.PenStyle.DashLine)
            dot.setPen(pen)
